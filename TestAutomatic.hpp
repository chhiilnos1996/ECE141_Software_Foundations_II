...
namespace ECE141 {
  ...
  void insertBooks(std::ostream &anOut,
    ...
    bool doCacheTest(CacheType aType, size_t aCapacity) {
      ...
      std::cout << "Zero cache done" << std::endl;
      ...
      if(theResult) {
        ...
        if(theResult) {
          double theAvgTime2=std::accumulate(
                  theTimes.begin(), theTimes.end(), 0.0) / theTimes.size();
                  std::cout << "No cache time: " << theAvgTime << " Cache time: " << theAvgTime2 << std::endl;
      std::cout << "Time success? " << (theAvgTime2<theAvgTime) << std::endl;
